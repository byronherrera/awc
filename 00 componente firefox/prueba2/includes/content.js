(function (win, doc) {
    ...
    var imageSendTo = function (sf) {
        ...
        for (; i < hosts.length; ++i) if (sf.host === i || sf.host === void 0 && hosts[i][0][0] === "+") urls.push(hosts[i][1].replace("%url", encodeURIComponent(sf.url)).replace("%raw_url",
            sf.url));
    ...
    var checkIMG = function (node) {
        ...
        if (nname === "IMG" || node.type === "image" || nname === "EMBED") return node.src; else if (nname === "CANVAS") return node.toDataURL(); else if (nname === "OBJECT" && node.data) return node.data; else if (nname === "AREA") {
    ...
    var onMouseDown = function (e) {
        ...
        if (cfg.hz.actTrigger === "m2") {
            if (PVI.fireHide && d) PVI.SRC = {"m2": PVI.SRC === null ? PVI.TRG.IMGS_c_resolved : PVI.SRC.m2 || PVI.SRC};
            PVI.freeze = cfg.hz.deactivate
    ...
    var onContextMenu = function (e) {
        ...
        if (i) pdsp(e); else if (e.target === PVI.CNT) pdsp(e, false); else if (e.ctrlKey && !elapsed && !e.shiftKey && !e.altKey && cfg.tls.opzoom && PVI.state < 2 && (i = checkIMG(e.target) || checkBG(win.getComputedStyle(e.target).backgroundImage))) {
    ...
    var PVI = {
        ...
        create: function () {
            ...
            if (cfg.hz.hideIdleCursor >= 50) {
                ...
                PVI.DIV.addEventListener("mousemove", function (e) {
                    ...
                    if (PVI.timers.cursor_hide) clearTimeout(PVI.timers.cursor_hide); else PVI.CNT.style.cursor = "";
        ...
        createCAP: function () {
            ...
            buildNodes(PVI.CAP, [{
                tag: "b",
                attrs: {style: "display: none; " + platform["transition_css"] + ": background-color .1s; border-radius: 3px; padding: 0 2px"}
            }, " ", {tag: "b", attrs: {style: "display: " + (cfg.hz.capWH ? "inline-block" : "none")}}, " ", {
                tag: "span", attrs: {
                    style: "color: inherit; display: " + (cfg.hz.capText ?
                        "inline-block" : "none")
                }
            }]);
            ...
            PVI.CAP.style.cssText = "left:0; right:auto; display:block; cursor:default; position:absolute; width:auto; height:auto; border:0; white-space: " + (cfg.hz.capWrapByDef ? "pre-line" : "nowrap") + '; font:13px/1.4em "Trebuchet MS",sans-serif; background:rgba(' +
                (n ? "255,255,255,.95" : "0,0,0,.75") + ") !important; color:#" + (n ? "000" : "fff") + " !important; box-shadow: 0 0 1px #" + (n ? "666" : "ddd") + " inset; padding:0 4px; border-radius: 3px";
        ...
        onAttrChange: function (e) {
            ...
            switch (e.attrName) {
                ...
                case "title":
                case "alt":
                    if (target === PVI.TRG) PVI.nodeToReset = target; else PVI.resetNode(target);
                    target.removeEventListener("DOMAttrModified", PVI.onAttrChange)
            }
            e.stopPropagation()
        },
        listen_attr_changes: function (node) {
            if (PVI.mutObserver) PVI.mutObserver.observe(node, PVI.mutObserverConf); else node.addEventListener("DOMAttrModified", PVI.onAttrChange)
        },
        ...
        getImages: function (el) {
            ...
            if (isHTMLElement) if (el.childElementCount >
                ...
                if (imgs.childElementCount && imgs.childElementCount < 4) if (imgs.firstElementChild.localName === "img") imgs = imgs.firstElementChild; else if (imgs.lastElementChild.localName === "img") imgs = imgs.lastElementChild;
            ...
            if (!imgs.imgSRC_o && el.localName === "image") {
                ...
                if (imgs.imgSRC_o) imgs.imgSRC_o = PVI.normalizeURL(imgs.imgSRC_o); else delete imgs.imgSRC_o
            }
            if (imgs.imgSRC_o) {
                if (!isHTMLElement) imgs.imgSRC_o = PVI.normalizeURL(imgs.imgSRC_o); else if (el.naturalWidth > 0 && el.naturalWidth < 3 || el.naturalHeight > 0 && el.naturalHeight < 3) imgs.imgSRC_o = null;
                if (imgs.imgSRC_o) imgs.imgSRC =
                    imgs.imgSRC_o.replace(PVI.rgxHTTPs, "")
            }
            if (!isHTMLElement) return imgs.imgSRC ? imgs : null;
            if (el.style.backgroundImage[0] === "u") imgs.imgBG_o = el.style.backgroundImage; else if (el.parentNode) {
                p = el.parentNode;
        ...
        _replace: function (rule, addr, http, param, to, trg) {
            ...
            if (typeof to === "function") {
                if (r ===
                    "") return 2; else if (r === "null") return null;
                if (r.indexOf("\n", 7) > -1) {
                    ...
                    for (i = 0; i < r.length; ++i) {
                        ...
                        if (Array.isArray(r[i])) ret = ret.concat(r[i]); else ret.push(r[i])
                    }
        ...
        replace: function (rule, addr, http, param, trg) {
            ...
            if (Array.isArray(rule.to)) {
                ...
                for (i = 0; i < rule.to.length; ++i) {
                    ...
                    if (Array.isArray(j)) ret = ret.concat(j); else ret.push(j)
                }
            } else if (rule.to) ret = PVI._replace(rule, addr, http, param, rule.to, trg); else ret = PVI.httpPrepend(addr, http);
            return ret
        },
        toFunction: function (rule, param, inline) {
            if (typeof rule[param] !== "function" && (inline ? /^:\s*\S/ : /^:\n\s*\S/).test(rule[param])) try {
                rule[param] =
                    Function("var $ = arguments; " + (inline ? "return " : "") + rule[param].slice(1)).bind(PVI)
            } catch (ex) {
        ...
        httpPrepend: function (url, preDomain) {
            ...
            if (url[1] === "/") if (url[0] === "/") url = PVI.pageProtocol + url; else if (url[0] === "#" && url[2] === "/") url = "#" + PVI.pageProtocol + url.slice(1);
            return url
        },
        ...
        resolve: function (URL, rule, trg, nowait) {
            ...
            if (cfg.sieve[rule.id].res ===
                1) rule.req_res = true; else if (rule.skip_resolve) if (typeof cfg.sieve[rule.id].res === "function") {
                params.url = [URL];
        ...
        find: function (trg, x, y) {
            ...
            do {
                if (n.nodeType !== void 0) if (n.nodeType !== 1 || n === doc.body) break; else if (n.localName !== "a") continue;
                if (!n.href) {
                ...
                for (i = 0; rule = cfg.sieve[i]; ++i) {
                    ...
                    if (ret === 1) return 1; else if (ret === 2) ret = false;
            ...
            if (!ret && ret !== null) {
                ...
                if (imgs && (imgs.imgSRC || imgs.imgBG)) {
                    ...
                    for (; rule = cfg.sieve[i]; ++i) if (use_img[0] || rule.img && (imgs.imgSRC && rule.img.test(imgs.imgSRC) || imgs.imgBG && (use_img[1] = rule.img.test(imgs.imgBG)))) {
                        ...
                        if (ret === 1) return 1; else if (ret === 2) return false;
                        if (trg.nodeType === 1) {
                            attrModNode = trg;
            ...
            if (rule && rule.loop && typeof ret === "string" && rule.loop & (use_img ? 2 : 1)) {
                ...
                if (ret) ret = Array.isArray(ret) ? ret.concat(rule) : [ret, rule]; else if (ret !== null) ret = rule
            }
            ...
            if (ret && (typeof ret === "string" || Array.isArray(ret))) {
                ...
                for (i = 0; i < ret.length; ++i) {
                    ...
                    if (tmp_el === true) tmp_el = 1; else if (tmp_el === 1) ret.splice(i--, 1)
                }
                if (!ret.length) if (trg.IMGS_fallback_zoom) {
                    ret = trg.IMGS_fallback_zoom;
                    delete trg.IMGS_fallback_zoom
                } else ret = false; else if (ret.length === 1) ret = ret[0][0] ===
                "#" ? ret[0].slice(1) : ret[0]
            }
            if (trg.nodeType !== 1) return ret;
            imgFallbackCheck:if (trg.localName === "img" && trg.hasAttribute("src")) {
                if (ret) if (ret === (trg.currentSrc || trg.src) && (!n || !n.href || n !== trg)) use_img = ret = false; else if (typeof use_img === "number") use_img = 3;
                if (rgxIsSVG.test(trg.currentSrc || trg.src)) break imgFallbackCheck;
                if (trg.parentNode.localName === "picture") tmp_el = trg.parentNode.querySelectorAll("[srcset]"); else if (trg.hasAttribute("srcset")) tmp_el = [trg]; else tmp_el = [];
                rule = {
                ...
                for (i = 0; i < tmp_el.length; ++i) {
                    ...
                    while (j--) {
                        ...
                        if (descriptor === "x") srcItem[1] = trg.naturalWidth * srcItem[1].slice(0, -1); else if (descriptor === "w") srcItem[1] = parseInt(srcItem[1], 10); else continue;
                ...
                if (rule.src && PVI.isEnlargeable(trg, rule)) rule = rule.src; else if (PVI.isEnlargeable(trg)) rule = trg.currentSrc || trg.src; else rule = null;
                var oParent = trg;
                i = 0;
                do {
            ...
            if (!trg.IMGS_caption) if (trg.alt && trg.alt !== trg.src && trg.alt !== imgs) trg.IMGS_caption = trg.alt; else if (tmp_el && cfg.hz.capLinkText) trg.IMGS_caption = tmp_el;
            if (trg.IMGS_caption) if (!cfg.hz.capLinkText && trg.IMGS_caption === tmp_el || trg.IMGS_caption === trg.href) delete trg.IMGS_caption; else PVI.prepareCaption(trg, trg.IMGS_caption);
            if (attrModNode) PVI.listen_attr_changes(attrModNode);
            return ret
        },
        ...
        show: function (msg, delayed) {
            ...
            if (msg) {
                ...
                if (PVI.state < 3 || PVI.LDR_msg) {
                    PVI.LDR_msg = null;
                    win.addEventListener(platform["wheel"], PVI.wheeler, {capture: true, passive: false})
                }
                if (msg === true) {
                    PVI.BOX = PVI.DIV;
                ...
                box.display = "block";
                if (box.opacity === "0" && (PVI.BOX === PVI.DIV && PVI.anim.opacity || PVI.BOX === PVI.LDR && cfg.hz.LDRanimate)) if (PVI.state === 2) PVI.anim.opacityTransition(); else setTimeout(PVI.anim.opacityTransition, 0);
            ...
            if (msg === void 0 && PVI.state === 4 || msg === true) {
                ...
                if (PVI.TRG.IMGS_SVG) {
                    ...
                    w = h[0];
                    h = h[1]
                } else if (w = PVI.CNT.naturalWidth) h = PVI.CNT.naturalHeight; else msg = true
            }
            if (PVI.fullZm) {
                if (!PVI.BOX) PVI.BOX =
                    PVI.LDR;
            ...
            if (msg === false) {
                ...
                var fs = cfg.hz.fullspace || cfg.hz.placement === 2,
                    cap_size = PVI.CAP && PVI.CAP.overhead && !(PVI.DIV.curdeg % 360) && PVI.CAP.state !== 0 && (PVI.CAP.state === 2 ||
                        PVI.TRG.IMGS_caption && cfg.hz.capText || PVI.TRG.IMGS_album || cfg.hz.capWH) ? PVI.CAP.overhead : 0,
                    vH = box["wm"] + (rot ? box["hpb"] : box["wpb"]),
                    hH = box["hm"] + (rot ? box["wpb"] : box["hpb"]) + cap_size,
                    vW = Math.min(w, (fs ? winW : x < rSide ? rSide : x) - vH), hW = Math.min(w, winW - vH);
                vH = Math.min(h, winH - hH);
                hH = Math.min(h, (fs ? winH : y < bSide ? bSide : y) - hH);
                if ((fs = vW / ratio) > vH) vW = vH * ratio; else vH = fs;
                if ((fs = hH * ratio) > hW) hH = hW / ratio; else hW = fs;
                if (hW > vW) {
                    w = Math.round(hW);
                    h = Math.round(hH)
                } else {
            ...
            switch (typeof idx) {
                ...
                default:
                    if (/^[+-]?\d+$/.test(idx)) {
                        ...
                        s = album[0];
                        i = s + 1;
                        for (i = i < album.length ? i : 1; i !== s; ++i < album.length ? 0 : i = 1) if (album[i][1] && idx.test(album[i][1])) {
                            idx = i;
                            break
                        }
        ...
        set: function (src) {
            ...
            if (Array.isArray(src)) {
                ...
                for (i = 0; i < src.length; ++i) {
                    if (!src[i]) continue;
                    if (src[i][0] === "#") src_HD.push(PVI.httpPrepend(src[i].slice(1))); else src_left.push(PVI.httpPrepend(src[i]))
                }
                if (!src_left.length) src_left = src_HD; else if (src_HD.length) {
            ...
            if (src.indexOf("&amp;") !==
                -1) src = src.replace(/&amp;/g, "&");
            if (rgxIsSVG.test(src)) PVI.TRG.IMGS_SVG = true; else delete PVI.TRG.IMGS_SVG;
            if (src === PVI.CNT.src) {
        ...
        content_onerror: function () {
            ...
            if (!src_res_arr || !src_res_arr.length) if (src_left) t.IMGS_c_resolved = src_left; else delete t.IMGS_c_resolved;
            if (src_left && !src_left.URL) PVI.set(src_left); else if (t.IMGS_HD_stack) {
        ...
        history: function (manual) {
            ...
            if (!platform.opera &&
                !platform.safari) {
                Port.send({cmd: "history", url: url, manual: manual});
                return
            }
            n = function () {
            ...
            if (PVI.TRG.IMGS_album) {
                ...
                if (PVI.CNT === PVI.IMG &&
                    ...
                    if (typeof PVI.TRG.IMGS_thumb !== "string") {
                        ...
                        if (PVI.TRG.hasAttribute("src")) PVI.TRG.IMGS_thumb = PVI.TRG.src; else if (PVI.TRG.childElementCount) {
                    ...
                    if (PVI.TRG.IMGS_thumb === PVI.IMG.src) {
                        ...
                        if (w && PVI.TRG.IMGS_thumb_ok) {
                            if (PVI.interlacer) w = PVI.interlacer.style; else {
        ...
        reset: function (preventImmediateHover) {
            ...
            if (PVI.fullZm) {
                ...
                win.addEventListener("mouseover",
                    PVI.m_over, true);
                doc.addEventListener(platform.wheel, PVI.scroller, {capture: true, passive: true});
        ...
        key_action: function (e) {
            ...
            if (shortcut.isModifier(e)) {
                ...
                if (PVI.fireHide && PVI.state < 3) if (cfg.hz.deactivate) PVI.m_over({"relatedTarget": PVI.TRG}); else PVI.load(PVI.SRC === null ? PVI.TRG.IMGS_c_resolved : PVI.SRC);
                PVI.freeze = !!cfg.hz.deactivate;
            ...
            if (!e.repeat) if (PVI.keyup_freeze_on) PVI.keyup_freeze(); else if (PVI.freeze === false && !PVI.fullZm && PVI.lastScrollTRG) PVI.mover({"target": PVI.lastScrollTRG});
            key = shortcut.key(e);
            ...
            if (e.altKey && e.shiftKey) {
                ...
                if (key === cfg.keys.hz_preload) win.top.postMessage({vdfDpshPtdhhd: "preload"},
                    ...
                    if (win.sessionStorage.IMGS_suspend) delete win.sessionStorage.IMGS_suspend; else win.sessionStorage.IMGS_suspend = "1";
                ...
                if (e.ctrlKey) {
                    if (PVI.state === 4) if (key === "C") {
                        if (!e.shiftKey && "oncopy" in doc) {
                            pv = true;
                            if (Date.now() - PVI.timers.copy < 500) key = PVI.TRG.IMGS_caption; else key = PVI.CNT.src;
                    ...
                    key === "+" || key === "=") PVI.resize(key === "-" ? "-" : "+"); else if (key === "Tab") {
                ...
                } else if (key === "Esc") if (PVI.CNT === PVI.VID && (win.fullScreen || doc.fullscreenElement || topWinW === win.screen.width && topWinH === win.screen.height)) pv = false; else PVI.reset(true);
                else if (key === cfg.keys.hz_fullZm || key === "Enter") if (PVI.fullZm) if (e.shiftKey) PVI.fullZm = PVI.fullZm === 1 ? 2 : 1; else PVI.reset(true); else {
                    ...
                    if (PVI.CNT === PVI.VID) {
                        ...
                        } else if (PVI.VID.paused) PVI.VID.play(); else PVI.VID.pause();
                        else if (key === "Up" || key === "Down") if (e.shiftKey) PVI.VID.playbackRate *= key === "Up" ? 4 / 3 : .75; else pv = null; else if (!e.shiftKey && (key === "PgUp" || key === "PgDn")) if (PVI.VID.audio) PVI.VID.currentTime += key === "PgDn" ? 4 : -4; else {
                            PVI.VID.pause();
                    ...
                    if (!pv && PVI.TRG.IMGS_album) {
                        switch (key) {
                            case "End":
                                if (e.shiftKey && (pv = prompt("#", PVI.stack[PVI.TRG.IMGS_album].search || "") || null)) PVI.stack[PVI.TRG.IMGS_album].search = pv; else pv = false;
                                break;
                            case "Home":
                                pv = true;
                                break;
                ...
                } else if (key === cfg.keys.mOrig || key === cfg.keys.mFit || key === cfg.keys.mFitW || key === cfg.keys.mFitH) PVI.resize(key); else if (key === cfg.keys.hz_fullSpace) {
                    cfg.hz.fullspace = !cfg.hz.fullspace;
                    PVI.show()
                } else if (key === cfg.keys.flipH) flip(PVI.CNT, 0); else if (key === cfg.keys.flipV) flip(PVI.CNT, 1); else if (key ===
                    ...
                    if (PVI.fullZm) PVI.m_move(); else PVI.show()
                    ...
                    switch (PVI.CAP.state) {
                        ...
                        case 2:
                            ...
                            break;
                        default:
                            key = cfg.hz.capWH && cfg.hz.capText ?
                                0 : 2
                    }
                    PVI.CAP.state = key;
                    PVI.CAP.style.display = "none";
                ...
                } else if (key === cfg.keys.hz_history) PVI.history(e.shiftKey); else if (key === cfg.keys.send) {
                    ...
                    cfg.keys.prefs) {
                    Port.send({"cmd": "open", "url": "options.html#settings"});
        ...
        fzClickAct: function (e) {
            ...
            } else if (e.target === PVI.VID) if ((e.offsetY || e.layerY || 0) < Math.min(PVI.CNT.clientHeight - 40, 2 * PVI.CNT.clientHeight / 3)) PVI.reset(true); else {
                if ((e.offsetY || e.layerY || 0) < PVI.CNT.clientHeight -
                    40 && (e.offsetY || e.layerY || 0) > 2 * PVI.CNT.clientHeight / 3) if (PVI.VID.paused) PVI.VID.play(); else PVI.VID.pause()
            } else PVI.reset(true);
        ...
        scroller: function (e) {
            if (e) {
                if (PVI.fullZm) return;
                if (!e.target.IMGS_) if (PVI.lastScrollTRG && PVI.lastScrollTRG !== e.target) PVI.lastScrollTRG = false; else if (PVI.lastScrollTRG !== false) PVI.lastScrollTRG = e.target
            }
        ...
        wheeler: function (e) {
            ...
            if (PVI.state > 2 && d >= 20) if (e.timeStamp - (PVI.lastScrollTime ||
                0) < d) d = null; else PVI.lastScrollTime = e.timeStamp;
        ...
        resize: function (x, xy_img) {
            ...
            viewportDimensions();
            if (rot) s.reverse();
            if (x === k.mFit) if (winW / winH < s[0] / s[1]) x = winW > s[0] ? 0 : k.mFitW; else x = winH > s[1] ? 0 : k.mFitH;
            switch (x) {
                case k.mFitW:
                    winW -= PVI.DBOX["wpb"];
                    s[1] *= winW / s[0];
        ...
        m_over: function (e) {
            ...
            trg = e.target;
            cache = trg.IMGS_c;
            if (!cache) if (trg.IMGS_c_resolved) src = trg.IMGS_c_resolved; else PVI.TRG = trg;
            if (cache || src || (src = PVI.find(trg, e.clientX, e.clientY)) || src === null) {
                ...
                if (delay) PVI.timers.preview = setTimeout(PVI.load, delay); else PVI.load(PVI.SRC)
            } else {
                trg.IMGS_c = true;
                PVI.TRG = null;
                if (PVI.fireHide) PVI.hide(e)
        ...
        preload: function (e) {
            ...
            nodes = function () {
                ...
                while (node = PVI.preloading.shift()) {
                    ...
                    if (src = PVI.find(node)) {
                        ...
                        if (Array.isArray(src)) {
                            var i, img = new Image;
                            img.IMGS_src_arr = [];
                            for (i = 0; i < src.length; ++i) if (cfg.hz.hiRes && src[i][0] === "#") img.IMGS_src_arr.push(src[i].slice(1)); else if (src[i][0] !== "#") img.IMGS_src_arr.push(src[i]);
                ...
                PVI.resolve_delay = 0;
                if (PVI.preloading.length) PVI.timers.preload = setTimeout(nodes, 300); else delete PVI.timers.preload
            };
        ...
        toggle: function (disable) {
            if (PVI.state || disable === true) PVI.init(null, true); else if (cfg) PVI.init();
            else Port.send({cmd: "hello", no_grants: true})
        },
        onWinResize: function () {
            viewportDimensions();
            if (PVI.state < 3) return;
            if (!PVI.fullZm) PVI.show(); else if (PVI.fullZm === 1) PVI.m_move()
        },
        winOnMessage: function (e) {
            ...
            if (cmd === "toggle" || cmd === "preload" || cmd === "isFrame") {
                ...
                if (d.album) PVI.album(d.album); else PVI.set(d.src)
            }
        },
        onMessage: function (d) {
            ...
            if (d.cmd === "resolved") {
                ...
                if (Array.isArray(d.m)) if (d.m.length) {
                    ...
                    if (cfg.hz.capText && d.m[0]) if (d.m[1]) PVI.prepareCaption(trg, d.m[1]); else if (cfg.hz.capLinkText && trg.IMGS_caption) d.m[1] = trg.IMGS_caption;
                    d.m = d.m[0]
                } else d.m = null; else if (typeof d.m !== "object" && typeof d.m !== "string") d.m = false;
                if (d.m) {
                    if (!d.noloop && !trg.IMGS_album &&
                        ...
                        if (d.m === null || d.m === 1) return d.m; else if (d.m === false) {
                            if (!d.return_url) PVI.show("R_res");
                            return d.m
                        }
                    }
                    if (d.return_url) return d.m;
                    if (trg === PVI.TRG) if (trg.IMGS_album) PVI.album(d.idx || "1"); else PVI.set(d.m); else {
            ...
            } else if (d.cmd === "toggle" || d.cmd === "preload") win.top.postMessage({vdfDpshPtdhhd: d.cmd}, "*"); else if (d.cmd === "hello") {
                var e = !!PVI.DIV;
                PVI.init(null, true);
                PVI.init(d);
                if (e) PVI.create()
        ...
        init: function (e, deinit) {
            if (deinit) {
                ...
                if (e !== void 0) {
                    ...
                    if (doc.readyState === "loading") doc.addEventListener("DOMContentLoaded", pageLoaded); else pageLoaded()
                } else if (!cfg) {
                    PVI.initOnMouseMoveEnd();
                    return
                }
