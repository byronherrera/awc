...
function select()
{
    ...
    $query = "SELECT * FROM amc_denuncias $where ORDER BY prosesado, fecha LIMIT $start, $limit";
...
function selectForm()
{
    ...
    $query = "SELECT * FROM amc_denuncias WHERE id = $id";
    if ($databaseAMC->Query($query)
    ) {
...
function negar()
{
    ...
    $id = (int)$_POST ['id'];
    $databaseAMC->Query("UPDATE amc_denuncias SET confirmed='false' WHERE (`id`='$id')");
    $query = "UPDATE amc_denuncias SET prosesado='true' WHERE (`id`='$id')";

    $mensaje = getmensaje ('negar');
...
function getmensaje ($opcion , $nombre= '' ,  $codigo_tramite='', $id='' ){
    switch ($opcion) {
        case 'aprobar' :
...
<div style="float: right; clear: both; width: 100%;"><img style="float: right;" src="http://agenciadecontrol.quito.gob.ec/images/logoamc.png" alt="" width="30%" /></div>
<div style="clear: both; margin: 50px 10%; float: left;">
<p>Estimado usuario gracias por escribirnos, su denuncia no la podemos realizar por el incumplimiento de alguna de estas causas: <br><br>
