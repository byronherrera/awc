...
function comboOrdenanzasTemas()
{
    global $os;
    $os->db->conn->query("SET NAMES 'utf8'");
    $sql = "SELECT id, CONCAT (nombre, ' - ', nombre_completo) AS nombre FROM amc_ordenanzas_temas WHERE activo = 1 ORDER BY orden";
    $result = $os->db->conn->query($sql);
    $data = array();
    while ($row = $result->fetch(PDO::FETCH_ASSOC)) {
        $data[] = $row;
...
function comboPersonalResolucion()
{
    global $os;
    $os->db->conn->query("SET NAMES 'utf8'");
...
function comboPersonalEjecucion()
{
    global $os;
    $os->db->conn->query("SET NAMES 'utf8'");
    $sql = "SELECT a.id, CONCAT(a.first_name,' ',a.last_name) AS nombre FROM qo_members a
            WHERE id in (SELECT qo_members_id FROM qo_groups_has_members where qo_groups_id=73)
            AND active = 1 ORDER BY a.first_name ASC,a.last_name ASC";
//    echo $sql;
    $result = $os->db->conn->query($sql);
    $data = array();
...
switch ($_GET['tipo']) {
    ...
    case 'ordenanzas' :
        comboOrdenanzas();
        break;
    case 'ordenanzastemas' :
        comboOrdenanzasTemas();
        break;
    ...
    case 'personalsecretaria' :
        ...
        break;
    case 'personalresolucion' :
        comboPersonalResolucion();
        break;
    case 'personalejecucion' :
        comboPersonalEjecucion();
        break;
