...
function selectDetalleInspecciones()
{
    ...
    $orderby = 'ORDER BY id DESC';
        ...
        $sql = "SELECT * FROM amc_inspeccion_nio $where $orderby LIMIT $start, $limit";
...
function insertDetalleInspecciones()
{
    ...
    $sql = "INSERT INTO amc_inspeccion_nio($cadenaCampos)
	values($cadenaDatos);";
...
function updateDetalleInspecciones()
{
    ...
    $sql = "UPDATE amc_inspeccion_nio SET  $cadenaDatos  WHERE amc_inspeccion_nio.id = '$data->id' ";
    $sql = $os->db->conn->prepare($sql);
    $sql->execute();
...
function deleteDetalleInspecciones()
{
    ...
    $sql = "DELETE FROM amc_inspeccion_nio WHERE id = $id";
    $sql = $os->db->conn->prepare($sql);
    $sql->execute();
    echo json_encode(array(
        "success" => $sql->errorCode() == 0,
