...
if ($where == '') {
    $where = "WHERE id_persona_encargada REGEXP '^[0-9]+$'  ";
} else {
    $where = $where . " AND id_persona_encargada REGEXP '^[0-9]+$'  ";
}

$os->db->conn->query("SET NAMES 'utf8'");

$sql = "SELECT  funcionario, 
					(SELECT CONCAT(qo_members.last_name, ' ', qo_members.first_name) 
            FROM qo_members  WHERE qo_members.id = funcionario ) nombre, 
						(SELECT COUNT(*)
						FROM amc_operativos as f 
						$where 
						AND f.id_persona_encargada = funcionario) as encargado,
						 SUM(horasencargado) horasencargado,
					 (SELECT COUNT(*) FROM `procesos-amc`.`amc_operativos_personal` WHERE `id_member` = funcionario 
						AND id_operativo IN (SELECT id 
						FROM amc_operativos as c
						$where) 
						AND asistencia = 1
						) totalapoyo, 
						SUM(horasapoyo) horasapoyo
...
FROM
amc_operativos AS m
INNER JOIN amc_operativos_personal ON amc_operativos_personal.id_operativo = m.id
$where
) as b    
...
$objPHPExcel->getActiveSheet()->setCellValue('A' . $filaTitulo1, "TOTAL HORAS POR FUNCIONARIO");
...
$objPHPExcel->getActiveSheet()->setCellValue('A' . $filacabecera, 'NUM');
$objPHPExcel->getActiveSheet()->setCellValue('B' . $filacabecera, 'Funcionario');
$objPHPExcel->getActiveSheet()->setCellValue('C' . $filacabecera, 'Operativos Responsable');
$objPHPExcel->getActiveSheet()->setCellValue('D' . $filacabecera, 'Horas responsable');
$objPHPExcel->getActiveSheet()->setCellValue('E' . $filacabecera, 'Operativos Apoyo ');
$objPHPExcel->getActiveSheet()->setCellValue('F' . $filacabecera, 'Horas Apoyo');
$objPHPExcel->getActiveSheet()->setCellValue('G' . $filacabecera, 'Total horas');
...
//    $fin = $date->format('H:i');
    ...
    $objPHPExcel->getActiveSheet()->setCellValue('C' . $filaInicio, $rowdetalle['encargado']);
    $objPHPExcel->getActiveSheet()->setCellValue('D' . $filaInicio, $rowdetalle['horasencargado']);
    $objPHPExcel->getActiveSheet()->setCellValue('E' . $filaInicio, $rowdetalle['totalapoyo']);
    $objPHPExcel->getActiveSheet()->setCellValue('F' . $filaInicio, $rowdetalle['horasapoyo']);
    $objPHPExcel->getActiveSheet()->setCellValue('G' . $filaInicio, $rowdetalle['horasencargado'] + $rowdetalle['horasapoyo']);
...
$objPHPExcel->getActiveSheet()->getStyle('G4:G40')->getFont()->setSize(9);
