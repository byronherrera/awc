...
$objPHPExcel->getActiveSheet()->setCellValue('H' . $filacabecera, 'Personal');
$objPHPExcel->getActiveSheet()->setCellValue('I' . $filacabecera, 'Funcionario responsable');
$objPHPExcel->getActiveSheet()->setCellValue('J' . $filacabecera, 'Telefono funcionario responsable');
$objPHPExcel->getActiveSheet()->setCellValue('K' . $filacabecera, 'Número veces asiste a la seman');
...
// recuperamos los nombres de los usuarios
    ...
    $cadena_personal = implode(", ", $nombresUsuarios);
    $rowdetalle['personal'] = regresaNombre($rowdetalle['id_persona_encargada']) ;

    $rowdetalle['personal2'] = $cadena_personal;

    $rowdetalle['telefono'] = regresaTelefono($rowdetalle['personal']);
    ...
    $objPHPExcel->getActiveSheet()->setCellValue('K' . $filaInicio, 'VECES');
...
function regresaTelefono($id_dato)
{
    ...
    if (trim($id_dato) != '') {
        $os->db->conn->query("SET NAMES 'utf8'");
        $sql = "SELECT telefono2 FROM amc_personal_distributivo WHERE CONCAT(TRIM(nombres),' ', TRIM(apellidos)) LIKE
 '$id_dato'"  ;
        $nombre = $os->db->conn->query($sql);
        if ($nombre) {
            $rownombre = $nombre->fetch(PDO::FETCH_ASSOC);
