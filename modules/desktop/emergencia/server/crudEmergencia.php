...
function selectDenuncias()
{
    ...
    $os->db->conn->query("SET NAMES 'utf8'");
    $sql = "SELECT * FROM amc_sancion_emergencia $where $orderby LIMIT $start, $limit";
    $result = $os->db->conn->query($sql);
    $data = array();
    while ($row = $result->fetch(PDO::FETCH_ASSOC)) {
        $data[] = $row;
    ...
    $sql = "SELECT count(*) AS total FROM amc_sancion_emergencia $where";
    $result = $os->db->conn->query($sql);
...
function validarCedulaCorreo($id)
{
    ...
    global $os;
    $os->db->conn->query("SET NAMES 'utf8'");
    $sql = "SELECT cedula, email FROM amc_sancion_emergencia WHERE id = $id";
    $result = $os->db->conn->query($sql);
...
function selectDenunciasForm()
{
    ...
    $id = (int)$_POST ['id'];
    $os->db->conn->query("SET NAMES 'utf8'");
    $sql = "SELECT *, (SELECT numero FROM amc_guias WHERE amc_guias.id = a.guia ) as guianumero, (SELECT COUNT(*) FROM amc_sancion_emergencia  b WHERE a.cedula = b.cedula and b.cedula <> '') as totaldocumentos FROM amc_sancion_emergencia as a  WHERE a.id = $id";
    $result = $os->db->conn->query($sql);
    $data = array();
    while ($row = $result->fetch(PDO::FETCH_ASSOC)) {
...
function updateDenunciasForm()
{
    ...
    $sql = "UPDATE amc_sancion_emergencia SET 
