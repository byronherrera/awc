...
$objPHPExcel->getActiveSheet()->setCellValue('A' . $filaTitulo1, "ACTA DE DONACIÓN");
...
// actualizar detalle idGuia
    ...
    $objPHPExcel->getActiveSheet()->setCellValue('A' . ($filaTitulo1 + 3), $rowdetalle['instructor'] . ", en calidad de Instructor Metropolitano de la Agencia Metropolitana de Control-AMC,");
    $objPHPExcel->getActiveSheet()->setCellValue('A' . ($filaTitulo1 + 4), "debidamente autorizado por el cargo que ostenta y, por la otra parte, ". $rowdetalle['representante_recibe_donacion']);
    ...
    $objPHPExcel->getActiveSheet()->setCellValue('A' . ($filacabecera + 10), "Firman para constancia de todo lo arriba indicado a las " . $time . " del día señalado");
    $objPHPExcel->getActiveSheet()->setCellValue('A' . ($filacabecera + 14), "ENTREGA CONFORME");
    $objPHPExcel->getActiveSheet()->setCellValue('A' . ($filacabecera + 15), "Nombre:");
    ...
    $objPHPExcel->getActiveSheet()->setCellValue('D' . ($filacabecera + 15), "CC:");
...
header('Content-Disposition: attachment;filename="Bodega-Acta-Donacion-MATIS-' . $today . '.xls"');
