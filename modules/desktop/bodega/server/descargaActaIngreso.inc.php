...
$objPHPExcel->getActiveSheet()->mergeCells('A' . $filaTitulo1 . ':B' . $filaTitulo1);
$objPHPExcel->getActiveSheet()->mergeCells('A' . $filaTitulo2 . ':B' . $filaTitulo2);
$objPHPExcel->getActiveSheet()->mergeCells('A' . $filaTitulo3 . ':B' . $filaTitulo3);

$objPHPExcel->getActiveSheet()->setCellValue('A' . $filaTitulo1, "ACTA DE INGRESO - BODEGA CENTRAL");
...
//$objPHPExcel->getActiveSheet()->setCellValue('C' . ($filascabecera + 2), "bodega");
...
$objPHPExcel->getActiveSheet()->setCellValue('B' . $filacabecera, 'DETALLE DE PRODUCTOS Y/O BIENES');
...
// actualizar detalle idGuia
    ...
    $objPHPExcel->getActiveSheet()->setCellValue('A' . ($filaTitulo1 + 2), 'Retiro No: '.$rowdetalle['codigo']);
    $objPHPExcel->getActiveSheet()->setCellValue('A' . ($filaTitulo1 + 3), 'Dirección y zona: '.$rowdetalle['direccion_retiro']);
    $objPHPExcel->getActiveSheet()->setCellValue('A' . ($filaTitulo1 + 4), 'ESPACIO PÚBLICO');
    $objPHPExcel->getActiveSheet()->setCellValue('A' . ($filaTitulo1 + 5), '____________________________________');
    ...
    $objPHPExcel->getActiveSheet()->setCellValue('A' . ($filadesprendibles ), 'Bodega Central <br> Retiro No: '.$rowdetalle['codigo']);
    //$objPHPExcel->getActiveSheet()->setCellValue('B' . ($filadesprendibles ), 'Bodega Central\rRetiro No: '.$rowdetalle['codigo']);
    $objPHPExcel->getActiveSheet()->setCellValue('A15', "Bodega Central \n Retiro No: ".$rowdetalle['codigo']);
    $objPHPExcel->getActiveSheet()->setCellValue('B15', "Bodega Central \n Retiro No: ".$rowdetalle['codigo']);
    $objPHPExcel->getActiveSheet()->getStyle('B15')->getAlignment()->setWrapText(true);

    //$objPHPExcel->getActiveSheet()->getStyle('B15')->getAlignment()->setWrapText(true);
    $objPHPExcel->getActiveSheet()->getStyle('B' . $filaInicio . ':A' . $filaInicio)->applyFromArray($styleArray);
    $filaInicio++;
}
...
$objPHPExcel->getActiveSheet()->getStyle('A1:A5')->applyFromArray(
    array(
        'alignment' => array(
...
$objPHPExcel->getActiveSheet()->getStyle('B' . $filacabecera . ':B' . ($filacabecera+1))->applyFromArray($styleArray);
$objPHPExcel->getActiveSheet()->getStyle('A' . $filadesprendibles . ':B' . ($filadesprendibles))->applyFromArray($styleArray);

$objPHPExcel->getActiveSheet()->getStyle('B11:B11')->getBorders()->getLeft()->setBorderStyle(PHPExcel_Style_Border::BORDER_THIN);
...
$objPHPExcel->getActiveSheet()->getStyle('A1:B10')->getFont()->setSize(14);
$objPHPExcel->getActiveSheet()->getStyle('A15:B15')->getFont()->setSize(14);
$objPHPExcel->getActiveSheet()->getStyle('A11:A100')->getFont()->setSize(14);
$objPHPExcel->getActiveSheet()->getStyle('B11:B14')->getFont()->setSize(11);
...
header('Content-Disposition: attachment;filename="Bodega-ActaIngreso-MATIS-' . $today . '.xls"');
